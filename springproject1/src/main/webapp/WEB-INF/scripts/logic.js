...
function delete_task(task_id) {
	let url = window.location.pathname.replace(/\/$/, "") + "/" + task_id;
    $.ajax({
        url: url,
        type: 'DELETE',
        success: function(data, status, xhr) {
            console.log("Response Headers: ", xhr.getAllResponseHeaders());
            console.log("Specific Header (Example): ", xhr.getResponseHeader("Content-Type"));
            alert("Task deleted successfully!");

        },
        error: function(xhr) {
            console.error("Error Headers: ", xhr.getAllResponseHeaders());
